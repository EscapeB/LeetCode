...
class Solution:
    def myAtoi(self, str: str) -> int:
        ...
        for i in str:
            if re.match("[^0-9]", i):
                if len(symbol_str) == 0 and len(digit_str) == 0:
                    if re.match("[+-]", i):
                        symbol_str += i
                    elif i == ' ':
                        continue
                ...
                digit_str += i
        # print(symbol_str, digit_str)
        if len(digit_str) == 0 or len(symbol_str) > 1:
            return 0
        else:
            result = int(symbol_str + digit_str)
